# Code to plot ball calls based on L2M Reports
# Load packages
library(tidyverse)
library(extrafont)
library(ggtext)
...
favor_chart <- badcalls_df %>%
  ...
  geom_vline(xintercept = seq(1, 70, 1), color = "floralwhite") +
  theme_owen() +
  ...
  labs(
    x = "Bad Calls That Have <span style='color:#008A80FF'>**Advantaged**</span><br>A Player From 2015-2023",
    y = ""
  )
...
oppose_chart <- badcalls_df %>%
  ...
  scale_x_continuous(limits = c(-70, 0), breaks = c(-70, -60, -50, -40, -30, -20, -10, 0), labels = c("70", "60", "50", "40", "30", "20", "10", "0")) +
  scale_y_discrete(position = "right") +
  geom_vline(xintercept = seq(-70, -1, 1), color = "floralwhite") +
  theme_owen() +
  coord_cartesian(clip = "off") +
  theme(
  ...
  labs(
    x = "Bad Calls That Have <span style='color:#BE4A47FF'>**Disadvantaged**</span><br>A Player From 2015-2023",
    y = ""
  )
...
p <- p +
  plot_annotation(
    title = "Bad Calls That Have <span style='color:#BE4A47FF'>**Disadvantaged**</span> Or <span style='color:#008A80FF'>**Advantaged**</span> A Player From 2015-23",
    subtitle = "A call is considered to have disadvantaged a player if it was an incorrect no call on their opponent or a incorrect call against them\nand visa versa for calls that advantage a player | Only showing players that have been ad/disadvantaged at least 20x",
...
# Save plot
ggsave("./figs/R/BadCalls_diff_rev_2015_23.png", p, w = 6, h = 8, dpi = 600)
